...
if isTrue "$COMMON_CONST_HISTORY_LOG"; then
  ...
  addHistoryLog "$COMMON_CONST_PROJECT_ACTION_BUILD" "$VAR_SCRIPT_START" "$VAR_SCRIPT_STOP" "$VAR_SCRIPT_RESULT" "$VAR_SRC_TAR_FILE_NAME" "$VAR_BIN_TAR_FILE_NAME" "$VAR_LOG_TAR_FILE_NAME"
  checkRetValOK
fi
