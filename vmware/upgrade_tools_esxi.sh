...
$SSH_CLIENT $COMMON_CONST_SCRIPT_USER@$PRM_HOST "if [ ! -d $CONST_HV_SSHKEYS_DIRNAME ]; then mkdir $CONST_HV_SSHKEYS_DIRNAME; cat > $CONST_HV_SSHKEYS_DIRNAME/authorized_keys; else cat > /dev/null;fi" < $HOME/.ssh/$COMMON_CONST_SSHKEYID.pub
...
#check tools exist
RET_VAL=$($SSH_CLIENT $COMMON_CONST_SCRIPT_USER@$PRM_HOST "if [ -d $COMMON_CONST_ESXI_TOOLS_PATH ]; then echo $COMMON_CONST_TRUE; fi;") || exitChildError "$RET_VAL"
if ! isTrue "$RET_VAL"
then #first install
  $SSH_CLIENT $COMMON_CONST_SCRIPT_USER@$PRM_HOST "mkdir $COMMON_CONST_ESXI_TOOLS_PATH; mkdir $COMMON_CONST_ESXI_PATCHES_PATH; mkdir $COMMON_CONST_ESXI_IMAGES_PATH"
  if ! isRetValOK; then exitError; fi
  #copy tools
  scp -r $COMMON_CONST_SCRIPT_DIRNAME/data $COMMON_CONST_SCRIPT_USER@$PRM_HOST:$COMMON_CONST_ESXI_TOOLS_PATH
  if ! isRetValOK; then exitError; fi
  #put templates
  put_template_tools_to_esxi "$PRM_HOST"
  #put ofvtool
  put_ovftool_to_esxi "$PRM_HOST"
else
  #get remote tools version
  REMOTE_TOOLS_VER=$($SSH_CLIENT $COMMON_CONST_SCRIPT_USER@$PRM_HOST "cat $COMMON_CONST_ESXI_DATA_PATH/version") || exitChildError "$REMOTE_TOOLS_VER"
  #get remote ovftools version
  REMOTE_OVFTOOLS_VER=$($SSH_CLIENT $COMMON_CONST_SCRIPT_USER@$PRM_HOST "$COMMON_CONST_ESXI_OVFTOOL_PATH/ovftool --version | awk '{print \$3}'") || exitChildError "$REMOTE_TOOLS_VER"
  if isNewLocalVersion "$LOCAL_TOOLS_VER" "$REMOTE_TOOLS_VER"
  then
    #remove old version templates
    $SSH_CLIENT $COMMON_CONST_SCRIPT_USER@$PRM_HOST "rm -fR $COMMON_CONST_ESXI_TEMPLATES_PATH"
    if ! isRetValOK; then exitError; fi
    #put new version templates
    put_template_tools_to_esxi "$PRM_HOST"
    #put new version tag
    scp -r $COMMON_CONST_SCRIPT_DIRNAME/data/version $COMMON_CONST_SCRIPT_USER@$PRM_HOST:$COMMON_CONST_ESXI_TOOLS_PATH/data/
    if ! isRetValOK; then exitError; fi
  fi
  if isNewLocalVersion "$LOCAL_OVFTOOLS_VER" "$REMOTE_OVFTOOLS_VER"
  then
    #remove old version ofvtool
    $SSH_CLIENT $COMMON_CONST_SCRIPT_USER@$PRM_HOST "rm -fR $COMMON_CONST_ESXI_TOOLS_PATH"
    if ! isRetValOK; then exitError; fi
