...
if ! isTrue "$RET_VAL"; then
  exitError "not found VM template ova package $COMMON_CONST_ESXI_IMAGES_PATH/$OVA_FILE_NAME on $PRM_HOST host. Exec 'create_vm_template.sh $PRM_VMTEMPLATE $PRM_HOST' previously"
fi
#check vm name
