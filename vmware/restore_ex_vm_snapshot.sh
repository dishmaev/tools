...
targetDescription "Restore standard snapshot of VM type $COMMON_CONST_VMWARE_VM_TYPE"
...
echoHelp $# 4 "<vmName> <snapshotName=\$ENV_PROJECT_NAME | \$COMMON_CONST_SNAPSHOT_TEMPLATE_NAME> [esxiHost=$COMMON_CONST_ESXI_HOST] [removeChildren=1]" \
...
if isTrue "$PRM_REMOVE_CHILD"; then
  VAR_CHILD_SNAPSHOTS_POOL=$(getChildSnapshotsPoolEx "$VAR_VM_ID" "$PRM_SNAPSHOT_NAME" "$VAR_SS_ID" "$PRM_ESXI_HOST") || exitChildError "$VAR_CHILD_SNAPSHOTS_POOL"
