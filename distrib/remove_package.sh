...
"myPackage $COMMON_CONST_PHOTONMINI_VM_TEMPLATE $COMMON_CONST_DEVELOP_SUITE $ENV_DISTRIB_REPO" \
...
[ "$PRM_VM_TEMPLATE" = "$COMMON_CONST_ORACLELINUXBOX_VM_TEMPLATE" ]; then
  ...
  if [ -f "$VAR_DISTRIB_REPO_DIR_PATH/x86_64/$VAR_SHORT_FILE_NAME" ]; then
    ...
    if [ -f "$VAR_DISTRIB_REPO_DIR_PATH/noarch/$VAR_SHORT_FILE_NAME" ]; then
